...
<body>
    ...
    <div class="wrapper">
        ...
        <form  class="layui-form" method="post" action=" ${pageContext.request.contextPath }/commitscore.do?username=<%=request.getParameter("username") %>&student_id=<%=request.getParameter("student_id") %>&teacher_id=<%=request.getParameter("teacher_id") %>&name=<%=request.getParameter("name") %>">
            <xblock> <p style="text-align:center;font-size:20px;">请为申报的项目打分</p> </xblock>
...
三大比赛作为高校深化创新创业教育改革的重要抓手；积极开展教学改革探索，把创新创业教育融入人才培养；推动赛事成果转化和产学研用紧密结合；以创新引领创业、创业带动就业，推动高校毕业生更高质量创业就业。
     ...
     <form action="jurydoUpload.do?username=<%=request.getParameter("username") %>&student_id=<%=request.getParameter("student_id") %>&teacher_id=<%=request.getParameter("teacher_id") %>&name=<%=request.getParameter("name") %>" enctype="multipart/form-data" method="post"mothod="post">
	<div class="layui-form-item">
		<div class="layui-input-inline">
             ...
             <a href="jurydownload.do?username=<%=request.getParameter("username") %>&student_id=<%=request.getParameter("student_id") %>&teacher_id=<%=request.getParameter("teacher_id") %>&name=<%=request.getParameter("name") %>">下载申报文件</a>
             </button>
