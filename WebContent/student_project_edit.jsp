...
<body>
    ...
    <div class="wrapper">
        ...
        <div class="page-content">
          <div class="content">
            <!-- 右侧内容框架，更改从这里开始 -->
            <form class="layui-form" method="post" action=" ${pageContext.request.contextPath }/applyproject.do?username=<%=request.getParameter("username") %>&project_id=<%=request.getParameter("project_id") %>&name=<%=request.getParameter("name") %>">
           
			<div class="footer">
              <div class="copyright">   <p style="text-align:center;font-size:20px;">请添加申报内容</p></div>  
			</div>
...
三大比赛作为高校深化创新创业教育改革的重要抓手；积极开展教学改革探索，把创新创业教育融入人才培养；推动赛事成果转化和产学研用紧密结合；以创新引领创业、创业带动就业，推动高校毕业生更高质量创业就业。
                ...
                <div class="layui-form-item layui-form-text">
                        ...
                        <textarea placeholder="写点啥呗，老铁"  name="project_content" autocomplete="off"
                        class="layui-textarea" required=""style="height: 80px;"></textarea>
                    
                </div>
               
                
  					
                    <button  class="layui-btn" lay-filter="add" required=""lay-submit=""autocomplete="off">
                                                     保存
                    </button>

                
            </form>
       <form action="studentdoUpload.do?username=<%=request.getParameter("username") %>&teacher_id=<%=request.getParameter("teacher_id") %>&project_id=<%=request.getParameter("project_id") %>&name=<%=request.getParameter("name") %>" enctype="multipart/form-data" method="post">
